 ...
 */
public class StaticWithQualifiersDependencyProvider implements DependencyProvider {
    private final Map<DependencyKey, Object> dependenciesByClassAndAnnotation;

    public StaticWithQualifiersDependencyProvider() {
        dependenciesByClassAndAnnotation = new ConcurrentHashMap<DependencyKey, Object>();
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <T> T inject(Class<T> cls, Annotation... qualifiers) {
        return (T) dependenciesByClassAndAnnotation.get(new DependencyKey(cls, qualifiers));
    }

    public <T> void register(T dependency, Class<T> cls, Annotation... qualifiers) {
        dependenciesByClassAndAnnotation.put(new DependencyKey(cls, qualifiers), dependency);
    }

    public void unregister(Class<?> cls, Annotation... qualifiers) {
        dependenciesByClassAndAnnotation.remove(new DependencyKey(cls, qualifiers));
    }
    ...
    public String toString() {
        return "StaticWithQualifiersDependencyProvider{dependenciesByClassAndAnnotation=" + dependenciesByClassAndAnnotation + "}";
    }
}
